...
module RubyLsp
  #: (String path) -> bool
  def rubygems_binstub?(path)
    return false unless File.exist?(path)
    content = File.read(path, encoding: "utf-8")
    content.include?("This file was generated by RubyGems") ||
      content.include?("Gem.activate_bin_path")
  rescue
    false
  end
  #: -> String
  def effective_bindir
    args = Gem.configuration["install"].to_s
    custom = args[/--bindir\s+(\S+)/, 1]
    custom || Gem.configuration[:bindir].to_s || Gem.bindir(Gem.user_dir)
  end
  # ✔️ 方法二：带 fallback 的安全写法
  # => "/home/lsq/.local/bin"（有自定义时返回自定义值）
  # => ".../gems/ruby/4.0.0/bin"（无自定义时回退到默认拼接路径）
  module_function :effective_bindir, :rubygems_binstub?
  ...
  class SetupBundler
    ...
    def run_bundle_install(bundle_gemfile = @gemfile)
      ...
      if @bundler_version
        ...
        @user_bundler_path = install_bundler_if_needed
      end
    ...
    def run_bundle_install_through_command(env)
      ...
      if @user_bundler_path && !Gem.win_platform? && File.exist?(bundler_path)
      base_command = "#{Gem.ruby} #{@user_bundler_path}".dup
      else
      ...
      if env["BUNDLER_VERSION"] && RubyLsp.rubygems_binstub?(bundler_path)
    ...
    def install_bundler_if_needed
      ...
      $stderr.puts("Ruby LSP> #{Gem::Specification.select{|s| s.name == 'bundler'}.map(&:version)}")
      requirement = Gem::Requirement.new(@bundler_version.to_s)
      return nil if Gem::Specification.any? { |s| s.name == "bundler" && requirement =~ s.version }
      $stderr.puts("Ruby LSP> #{requirement}")

      Gem.install("bundler", @bundler_version.to_s, install_dir: Gem.user_dir, bin_dir: RubyLsp.effective_bindir, env_shebang: true)
      File.join(RubyLsp.effective_bindir, "bundle")
    end
